  ...
  return (
    <footer className="w-full max-w-screen-2xl mx-auto mt-20 mb-20 px-4">
      <section className="w-full ">
        <div className="w-full flex flex-col gap-4 lg:gap-2 lg:flex-row lg:justify-between mb-8">
          <p className="text-[#FFD60A] text-[clamp12px,2vw,18px)] font-semibold">
      ...
      <section className="flex flex-col lg:flex-row justify-between gap-4 mt-5 ">
        <div>
          <Image
            ...
            height={90}
          />
          <p className="text-base text-[clamp(14px,2vw,16px)] text-white w-full max-w-[400px]">
        ...
        <div className="flex justify-between items-center lg:gap-16">
          <div className="flex flex-col justify-center">
            <h4 className=" text-[clamp(16px,3vw,20px)] text-white font-medium mb-2">
              Explore
            </h4>
            <ul>
              {exploreLinks.map((e, index) => (
                <li key={index}>
                  <a href={e.href} className="text-[clamp(14px,2vw,16px)]">
                    {e.name}
                  </a>
                </li>
              ))}
          ...
          <div className="flex flex-col justify-center">
            <ul>
              {servicesLinks.map((s, index) => (
                <li key={index}>
                  <a href={s.href} className="text-[clamp(14px,2vw,16px)]">
                    {s.name}
                  </a>
                </li>
              ))}
            ...
            {contactInfo.map((c, index) => {
              ...
              return (
                <li className="flex gap-3" key={index}>
                  <div>
                    <Icon className="w-8 h-8 md:w-10 md:h-10 text-[#FFD60A]"></Icon>
                  </div>
                  <div>
                    <p className="text-[clamp(14px,2vw,16px)]">{c.type}</p>
                    <p>{c.value}</p>
                  </div>
                </li>
              );
